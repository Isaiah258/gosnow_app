...
struct Recents: View {
    ...
    let onSummary: (SessionSummary) -> Void
    let onWillStop: () -> Void

    // 便捷 init：内部自建 VM
    init(
        onWillStop: @escaping () -> Void = {},
        onSummary:  @escaping (SessionSummary) -> Void = { _ in }
    ) {
    ...
    var body: some View {
        ...
        // ⬅️ 只有在 recording / paused 时才更新最高速度，idle 时不理会
        .onChange(of: vm.speedKmh) { _, v in
            switch vm.state {
            case .recording, .paused:
                if v > maxSpeedKmh { maxSpeedKmh = v }
            case .idle:
                // idle 状态下 speed 抖动一律无视（保持 0）
                break
            }
        }

        // ⬅️ 一旦状态变回 idle，就把最高速度清 0（比如 stop 之后）
